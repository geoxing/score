...
def get_etf_year_close_line(etf_info_map):
    ...
    return line
def get_etf_year_close_line_all(etf_info_map):
    all_data = {}
    for item in etf_info_map.keys():
        print(item)
        result_close_points= getClosePoint(item)
    ...
    for stock_code, close_point in all_close_points.items():
        ...
        etf_name = etf_info.get("name")  # 获取自定义名称或默认名称
        # 打印数据范围
        t_min_value = min(close_point)
        t_max_value = max(close_point)
        if count == 0:
            min_value = t_min_value
            max_value = t_max_value
            count = count + 1
        else:
            if t_min_value < min_value:
                min_value = t_min_value
    ...
    return line
def get_etf_year_close_line_overseas(etf_info_map):
    all_data = {}
    for item in etf_info_map.keys():
        print(item)
    ...
    return line
def get_etf_year_close_line_all_overseas(etf_info_map):
    all_data = {}
    for item in etf_info_map.keys():
        print(item)
        result_close_points= getClosePoint_overseas(item)
